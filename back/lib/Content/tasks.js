...
_.select = async function () {
    ...
    return {
        ...
        cnt: await this.db.select_scalar ('SELECT COUNT(*) FROM tasks LEFT JOIN task_notes ON (id_last_task_note = task_notes.id ) WHERE 1=1 AND (tasks.id_user IN (?)) AND tasks.fake = 0', params),
    }
